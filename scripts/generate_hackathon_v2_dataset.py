...
def main():
    ...
    if args.smoke_test:
        print("Running V2 smoke test mode (2 cases)...")
        cases = [
            ("case_0001", "DRAM", "moderate"),
            ("case_0032", "FinFET", "hard")
        ]
    else:
        print("Running full V2 dataset generation (60 cases)...")
        cases = []
        for i in range(1, 31):
    ...
    for idx, (case_id, arch, diff) in enumerate(cases):
        print(f"Generating V2 {case_id} ({arch}, {diff})...")
        seed = base_seed + idx
        
        gen = HackathonDatasetGenerator(seed=seed)
        # Pass version='v2' to enable macro-boundary sampling
        ref_img, search_img, meta = gen.generate_case(case_id, arch, diff, version="v2")
        ...
        manifest_rows.append({
            ...
            'reference_context_type': meta.reference_context_type,
            'macro_boundary_present': meta.macro_boundary_present,
