...
def validate():
    ...
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'dataset', 'hackathon_v2'))
    
    print("========================================")
    print("HACKATHON V2 DATASET VALIDATION")
    print("========================================\n")
    ...
    for cdir in case_dirs:
        ...
        seeds.add(meta['seed'])
        if 'reference_context_type' in meta:
            context_types.add(meta['reference_context_type'])
        
        # Check metadata completeness (V2 requires macro context fields)
        required_keys = ['gt_x', 'gt_y', 'gt_bbox', 'seed', 'architecture', 'difficulty',
                        ...
                        'reference_context_type', 'macro_boundary_present', 'periodicity_score',
                        'edge_density', 'gradient_energy', 'local_variance', 'reference_sampling_rule']
        if not all(k in meta for k in required_keys):
            meta_ok = False
            failed_cases.append((case_id, "Incomplete metadata for V2"))
        ...
        # Check macro context V2 constraints
        if meta.get('macro_boundary_present') is not True:
            macro_context_ok = False
            failed_cases.append((case_id, "Missing macro boundary flag"))
            
        if meta.get('periodicity_score', 1.0) > 0.95:
            macro_context_ok = False
            failed_cases.append((case_id, "Periodicity score too high (no macro transition)"))
            
        if meta.get('gradient_energy', 0) < 500 or meta.get('edge_density', 0) < 0.01:
            gradient_ok = False
            failed_cases.append((case_id, "Insufficient gradient/edge information"))

        # Reproducibility check
        if not reproducibility_checked:
            reproducibility_checked = True
            gen = HackathonDatasetGenerator(seed=meta['seed'])
            re_ref, re_search, re_meta = gen.generate_case(meta['case_id'], meta['architecture'], meta['difficulty'], version="v2")
    ...
    print(f"Macro context present: {'PASS' if macro_context_ok else 'FAIL'} ({len(context_types)} context types)")
    print(f"Gradient/Edge sufficiency: {'PASS' if gradient_ok else 'FAIL'}\n")
    ...
    if not macro_context_ok: global_pass = False
    if not gradient_ok: global_pass = False
    if not args.smoke_test and len(context_types) < 4: global_pass = False # Expect at least 4 context types in 60 cases
