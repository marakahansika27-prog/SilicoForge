...
def main():
    print("V4 GSPE NMS A/B EXPERIMENT (3000 CONTEXT)...")
    ...
    csv_out_path = os.path.join(reports_dir, 'GSPE_NMS_AB_EXPERIMENT.csv')
    md_out_path = os.path.join(docs_dir, 'GSPE_NMS_AB_EXPERIMENT_REPORT.md')
    ...
    # Always use 3000 context size (300x300 in search image)
    context_size = 3000
    nms_multipliers = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5]
    
    print(f"Loaded {len(cases)} positive cases. Running NMS multipliers: {nms_multipliers}")
    
    results_by_mult = {m: [] for m in nms_multipliers}
    csv_rows = []
    ...
    for idx, c in enumerate(cases):
        ...
        # Only need to extract reference once per case for the 3000 context size
        ref_img = extract_reference_size(meta, context_size)
        
        for mult in nms_multipliers:
            with Profiler("gspe_run") as p:
                gspe = SimulationGSPE(nms_mult=mult)
    ...
    with open(md_out_path, 'w') as f:
        f.write("# GSPE NMS A/B Experiment Report (3000 Context)\n\n")
        
        f.write("## 1. Accuracy by NMS Multiplier\n")
        f.write("| NMS Multiplier | Mean | Median | P90 | Max | <=1 | <=5 | <=10 | <=25 | <=50 | >50 |\n")
        f.write("|----------------|------|--------|-----|-----|-----|-----|------|------|------|-----|\n")
        
        for m in nms_multipliers:
            errs = [r['err'] for r in results_by_mult[m]]
            f.write(f"| {m}x | {np.mean(errs):.2f} | {np.median(errs):.2f} | {np.percentile(errs, 90):.2f} | {np.max(errs):.2f} | ")
        ...
        f.write("| NMS Multiplier | 1.0x Failures Recovered | 1.0x Successes Regressed |\n")
        f.write("|----------------|-------------------------|--------------------------|\n")
        ...
        for m in nms_multipliers:
            ...
            rec = 0
            reg = 0
            for r_base, r_curr in zip(results_by_mult[1.0], results_by_mult[m]):
                if r_base['err'] > 50.0 and r_curr['err'] <= 50.0:
                    rec += 1
                elif r_base['err'] <= 50.0 and r_curr['err'] > 50.0:
                    reg += 1
            f.write(f"| {m}x | {rec} | {reg} |\n")
            
        f.write("\n## 3. Failure Classifications (>50px)\n")
        f.write("| NMS Multiplier | NMS_FAILURE (GT Suppressed) | PERIODIC_ALIAS | TRUE_AMBIGUITY | GEOMETRY_COARSE_RANK_FAILURE | GEOMETRY_NOT_COVERED | SCORING_FAILURE | OTHER |\n")
        f.write("|----------------|-----------------------------|----------------|----------------|------------------------------|----------------------|-----------------|-------|\n")
        
        for m in nms_multipliers:
            cats = [r['cls'] for r in results_by_mult[m] if r['err'] > 50.0]
            c_nms = cats.count("NMS_FAILURE")
            c_alias = cats.count("PERIODIC_ALIAS")
            c_ambig = cats.count("TRUE_AMBIGUITY")
            c_coarse = cats.count("GEOMETRY_COARSE_RANK_FAILURE")
            c_nocov = cats.count("GEOMETRY_NOT_COVERED")
            c_score = cats.count("SCORING_FAILURE")
            c_other = len(cats) - sum([c_nms, c_alias, c_ambig, c_coarse, c_nocov, c_score])
            f.write(f"| {m}x | {c_nms} | {c_alias} | {c_ambig} | {c_coarse} | {c_nocov} | {c_score} | {c_other} |\n")
            
        f.write("\n## 4. Runtime per case (ms)\n")
        f.write("| NMS Multiplier | Mean Runtime |\n")
        f.write("|----------------|--------------|\n")
        for m in nms_multipliers:
            rts = [r['rt'] for r in results_by_mult[m]]
            f.write(f"| {m}x | {np.mean(rts):.2f} |\n")
            
        f.write("\n## Conclusion\n")
        f.write("Evaluate whether adjusting NMS suppression size meaningfully recovers remaining periodic aliases after context expansion, without triggering spatial regressions.\n")
